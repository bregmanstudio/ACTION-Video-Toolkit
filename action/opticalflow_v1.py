...
class OpticalFlow:
	...
	def _opticalflow_for_segment_from_onset_with_duration(self, onset_time=0, duration=-1):
		...
		return np.memmap(self.data_path, dtype='int32', mode='c', offset=onset_frame, shape=(dur_frames,COLS))
...
# 		self._process_movie(movie_file, data_file, mode='playback', display=True, offset=offset, duration=duration)
	...
	def _process_movie(self, **kwargs):
		...
		# set up memmap
		print 'ANALYZE!'
		fp = np.memmap(self.data_path, dtype='int32', mode='w+', shape=(dur_strides,COLS))
		print 'dur. strides: ', dur_strides
			...
			if len(self.tracks) > 0 and self.frame_idx % 3 == 0:
				...
				if self.frame_idx % ap['stride'] == 0:
					...
					try:
						...
						combo_bins = np.multiply(ctheta_bins, 8.0) + theta_bins

						bins_histo, bin_edges = np.histogram(combo_bins, COLS, (0., float(COLS - 1)))
						...
						fp[fd6] = np.zeros(COLS, dtype='int32')
