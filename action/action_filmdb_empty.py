actionDB = {
	...
	'Diary_of_a_Country_Priest': ['Diary_of_a_Country_Priest', 'RB', 0, 1951],
...
# 	'Le_Gai_Savior': ['Le_Gai_Savior', 'JLG', 1, 1969],
	...
	'Robinson_Crusoe': ['Robinson_Crusoe', 'JB', 1, 1954],
	'Rope': ['Rope', 'AH', 1, 1948],
...
def create_analysis_pool(directors, cflag):
	analysisPool = dict()
	for dir in directors:
		for entry in actionDB:
			# print actionDB[entry]
			if ((actionDB[entry][2] == cflag) or (cflag == 2)) and (actionDB[entry][1] in directors): 
				# print "ADD TO SET: ", entry
				try:
					analysisPool[actionDB[entry][1]].add(actionDB[entry][0])
				except KeyError:
					analysisPool[actionDB[entry][1]] = set([actionDB[entry][0]])
	return analysisPool


def films_for_directors(director):
	films = []
	for ttl in actionDB.keys():
		if (actionDB[ttl][1] == director):
			films += [ttl]
	return films
def films_for_directors_with_year(director):
	films = []
	for ttl in actionDB.keys():
		if (actionDB[ttl][1] == director):
			films += [[ttl, actionDB[ttl][3]]]
	return films
def films_for_years(years):
	return NotImplementedError
def all_black_and_white_films():
	return NotImplementedError
def all_color_films():
	return NotImplementedError
