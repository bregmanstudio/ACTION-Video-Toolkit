...
::
	...
	optflw = OpticalFlow('Psycho', action_dir='~/data/action')
	optflw.analyze_movie()
...
::
	...
	data = optflw.opticalflow_for_segment(segment_in_seconds)
...
class OpticalFlow:
	...
	def _opticalflow_for_segment_from_onset_with_duration(self, onset_time=0, duration=-1):
		...
		return np.memmap(self.data_path, dtype='float32', mode='c', offset=onset_frame, shape=(dur_frames,128))
...
# 		self._process_movie(movie_file, data_file, mode='playback', display=True, offset=offset, duration=duration)
	...
	def analyze_movie(self, offset=0, duration=-1, stride=6):
		"""
...
# 		self.fctrh = self.capture.get(cv.CV_CAP_PROP_FRAME_HEIGHT) / 2.0
	...
	def _process_movie(self, **kwargs):
		...
		# set up memmap
		print 'ANALYZE!'
		fp = np.memmap(self.data_path, dtype='float32', mode='w+', shape=(dur_strides,128))
		print 'dur. strides: ', dur_strides
...
# 							print np.max(combo_bins)
							...
							bins_histo, bin_edges = np.histogram(combo_bins, (16*8), weights=weighted) #(0., float((16*8) - 1)))
